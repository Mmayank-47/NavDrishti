...
scp -o LogLevel=ERROR -o StrictHostKeyChecking=no -i $KEY_PATH scripts/check_len.py "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_DIR}/scripts/"
ssh -o LogLevel=ERROR -o StrictHostKeyChecking=no -i $KEY_PATH "${REMOTE_USER}@${REMOTE_HOST}" "$PYTHON $REMOTE_DIR/scripts/check_len.py"
